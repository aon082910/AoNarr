...
export async function isBlocklisted(mediaItemId: number, releaseTitle: string): Promise<boolean> {
  return !!(await db
    .prepare("SELECT id FROM blocklist WHERE media_item_id = ? AND release_title = ?")
    .get(mediaItemId, releaseTitle));
}

export async function getBlocklistedTitles(mediaItemId: number): Promise<Set<string>> {
  const rows = (await db.prepare("SELECT release_title FROM blocklist WHERE media_item_id = ?").all(mediaItemId)) as {
