...
export function recordDiskUsageSamples(): void {
  const folders = db.prepare("SELECT id, path FROM root_folders").all() as { id: number; path: string }[];
  const today = new Date().toISOString().slice(0, 10);

  for (const folder of folders) {
    const alreadySampledToday = db
    ...
    try {
      ...
      db.prepare("INSERT INTO disk_usage_samples (root_folder_id, free_bytes, total_bytes) VALUES (?, ?, ?)").run(
        folder.id,
  ...
  db.prepare(`DELETE FROM disk_usage_samples WHERE sampled_at < datetime('now', '-${SAMPLE_RETENTION_DAYS} days')`).run();
}
...
export function getStorageForecast(rootFolderId: number): StorageForecast | null {
  const samples = db
