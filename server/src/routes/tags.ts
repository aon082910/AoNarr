...
tagsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    db.prepare(`INSERT INTO tags (name) VALUES (?) ON CONFLICT(name) DO UPDATE SET name = excluded.name`).run(name);
...
tagsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    db.prepare("UPDATE tags SET retention_days = ? WHERE id = ?").run(
