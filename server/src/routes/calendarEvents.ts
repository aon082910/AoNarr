...
calendarEventsRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    const rows = db.prepare("SELECT * FROM custom_calendar_events ORDER BY date").all() as any[];
...
calendarEventsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const row = db.prepare("SELECT * FROM custom_calendar_events WHERE id = ?").get(result.lastInsertRowid) as any;
...
calendarEventsRouter.delete(
  "/:id",
  asyncHandler(async (req, res) => {
    const result = db.prepare("DELETE FROM custom_calendar_events WHERE id = ?").run(req.params.id);
