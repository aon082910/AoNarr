...
shareLinksRouter.get(
  ...
  asyncHandler(async (req, res) => {
    res.json(db.prepare("SELECT * FROM share_links WHERE media_item_id = ? ORDER BY created_at DESC").all(req.params.id));
  })
);
...
shareLinksRouter.post(
  "/:id/share",
  asyncHandler(async (req, res) => {
    const item = db.prepare("SELECT id FROM media_items WHERE id = ?").get(req.params.id);
    ...
    db.prepare("INSERT INTO share_links (token, media_item_id, expires_at) VALUES (?, ?, ?)").run(
      token,
      req.params.id,
      expiresAt
    );
...
shareLinksRouter.delete(
  "/:id/share/:linkId",
  asyncHandler(async (req, res) => {
    db.prepare("DELETE FROM share_links WHERE id = ? AND media_item_id = ?").run(req.params.linkId, req.params.id);
    res.status(204).send();
  })
);
...
shareLinksPublicRouter.get(
  "/:token",
  asyncHandler(async (req, res) => {
    const link = db.prepare("SELECT * FROM share_links WHERE token = ?").get(req.params.token) as
