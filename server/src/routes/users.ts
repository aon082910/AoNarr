...
async function getAllowedTypes(userId: number): Promise<string[]> {
  return (
    (await db.prepare("SELECT media_type FROM user_library_access WHERE user_id = ?").all(userId)) as {
      media_type: string;
    }[]
  ).map((r) => r.media_type);
}
...
usersRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    const rows = (await db.prepare("SELECT * FROM users ORDER BY username").all()) as any[];
    res.json(await Promise.all(rows.map(async (r) => ({ ...userFromRow(r), allowedTypes: await getAllowedTypes(r.id) }))));
  })
);
  ...
  asyncHandler(async (req, res) => {
    ...
    if (b.maxPendingRequests !== undefined) {
      await db
        ...
        .run(b.maxPendingRequests === null ? null : Number(b.maxPendingRequests), req.params.id);
      permissionChanges.push(`max pending requests → ${b.maxPendingRequests ?? "unlimited"}`);
    }
    if (b.autoApprove !== undefined) {
      await db.prepare("UPDATE users SET auto_approve = ? WHERE id = ?").run(b.autoApprove ? 1 : 0, req.params.id);
    ...
    res.json({ ...userFromRow(row), allowedTypes: await getAllowedTypes(Number(req.params.id)) });
  })
);
...
usersRouter.delete(
  "/sessions/:token",
  asyncHandler(async (req, res) => {
    const session = (await db.prepare("SELECT user_id FROM sessions WHERE token = ?").get(req.params.token)) as
      | { user_id: number }
      | undefined;
    const result = await db.prepare("DELETE FROM sessions WHERE token = ?").run(req.params.token);
