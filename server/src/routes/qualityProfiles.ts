...
qualityProfilesRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    const rows = db.prepare("SELECT * FROM quality_profiles").all();
...
qualityProfilesRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const row = db.prepare("SELECT * FROM quality_profiles WHERE id = ?").get(result.lastInsertRowid);
...
qualityProfilesRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (sets.length > 0) {
      ...
      db.prepare(`UPDATE quality_profiles SET ${sets.join(", ")} WHERE id = ?`).run(...values);
    }
    const row = db.prepare("SELECT * FROM quality_profiles WHERE id = ?").get(req.params.id);
...
qualityProfilesRouter.delete(
  "/:id",
  asyncHandler(async (req, res) => {
    const result = db.prepare("DELETE FROM quality_profiles WHERE id = ?").run(req.params.id);
